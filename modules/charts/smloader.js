...
function parseDifficulty(difficulty, steps, notes, md, timedata, smChartArray) {
	...
	// rolls (4) or roll release (5) not implemented
...
function loadData(dataString) {
	...
	let wsRemoved = chartData[20].map(value => {
		// couldn't find a better way to remove whitespace from #NOTES but not anything else
		return value.replace(/\s/g, "");
	});

	chartData[20] = wsRemoved;
    
	let metadata = {};
	let timedata = {};
	let fieldName;

	chartData.forEach(function(field) {
